...
public class TicTacToe
{
    ...
    public static boolean checkValidForm(String coordinate){    
        if(coordinate.length()==3 && coordinate.substring(1,2).equals(",")){
            return true;
        }
        return false;
    }

    public static boolean checkValidNum(String[][] grid, String coordinate, int row, int col){
        if(row>=0 && row<=2 && col>=0 && col<=2 && grid[row][col].equals( "[ ]" )){
            return true;
        }
        return false;
    }

    public static boolean checkFull(String[][] grid){
        for (int row = 0; row < grid.length; row++) {
            for (int col = 0; col < grid[0].length; col++) {
